...
if ($instructionsOn) {
    ...
    Write-Output "`tIf these commands are successfully executed, registration is expected to occur within minutes. This"
    ...
    Write-Output "`t`t`ta. `$cep = Get-ExecutionPolicy`n"
    Write-Output "`t`t`tb. Set-ExecutionPolicy Bypass`n"
    Write-Output "`t`t`tc. Import-Module `"C:\Program Files\WindowsPowerShell\Modules\policyfileeditor`"`n"
    Write-Output "`t`t`td. Set-ExecutionPolicy `$cep`n"
...
if ($forceGPUpdate -eq "Y") { $cred = Get-Credential }
...
$comps | ForEach-Object {
    ...
    $Names_WU | ForEach-Object { 
        Try { 
            Set-PolicyFileEntry -Path "\\$currComp\$MachineDir" -Key $key -ValueName $_ `
    ...
    Try {
        Set-PolicyFileEntry -Path "\\$currComp\$MachineDir" -Key $key -ValueName ElevateNonAdmins -Data 1 -Type "DWORD" -ErrorAction Stop
    }
    Catch { $errorList.Add( @{ 'Hostname_ValueName' = "$($currComp):$($currValueName)" ; 'Exception' = $_.Exception.Message } ) }
        ...
        Try {
            switch ($_) {
                ...
                { $_ -eq "NoAutoUpdate" -or $_ -eq "ScheduledInstallDay" -or $_ -eq "AutoInstallMinorUpdates" -or $_ -eq "NoAUShutdownOption"-or $_ -eq "NoAUAsDeafultShutdownOption" }
                    { Set-PolicyFileEntry -Path "\\$currComp\$MachineDir" -Key $key -ValueName $_ -Data 0 -Type "DWORD" -ErrorAction Stop }
                { $_ -eq "AUOptions" }
                    { if ($AUOption -eq 2) 
                        { Set-PolicyFileEntry -Path "\\$currComp\$MachineDir" -Key $key -ValueName $_ -Data 2 -Type "DWORD" -ErrorAction Stop }
                        else
                        { Set-PolicyFileEntry -Path "\\$currComp\$MachineDir" -Key $key -ValueName $_ -Data 3 -Type "DWORD" -ErrorAction Stop }
                    }
                { $_ -eq "ScheduledInstallTime" }
                    { Set-PolicyFileEntry -Path "\\$currComp\$MachineDir" -Key $key -ValueName ElevateNonAdmins -Data 0x0a -Type "DWORD" -ErrorAction Stop }
                ## Works for Win 10, but not Win 7
                ...
                { $_ -eq "NoAutoRebootWithLoggedOnUsers" -or $_ -eq "RebootRelaunchTimeoutEnabled" -or $_ -eq "RescheduledWaitTimeEnabled" -or $_ -eq "DetectionFrequencyEnabled" -or
                    ...
                    { Set-PolicyFileEntry -Path "\\$currComp\$MachineDir" -Key $key -ValueName $_ -Data 1 -Type "DWORD" -ErrorAction Stop }
                { $_ -eq "RebootRelaunchTimeout" }
                    { Set-PolicyFileEntry -Path "\\$currComp\$MachineDir" -Key $key -ValueName $_ -Data 0x3F480 -Type "DWORD" -ErrorAction Stop }
                { $_ -eq "RescheduleWaitTime" } 
                    { Set-PolicyFileEntry -Path "\\$currComp\$MachineDir" -Key $key -ValueName $_ -Data 0x0f -Type "DWORD" -ErrorAction Stop }
                { $_ -eq "RebootWarningTimeout" }
                    { Set-PolicyFileEntry -Path "\\$currComp\$MachineDir" -Key $key -ValueName $_ -Data 0x1e -Type "DWORD" -ErrorAction Stop }
            }
        }
        Catch { $errorList.Add( @{ 'Hostname_ValueName' = "$($currComp):$($currValueName)" ; 'Exception' = $_.Exception.Message } ) }
    }
    ...
    if ($forceGPUpdate -eq "Y") {
        ...
        if ($env:COMPUTERNAME -ne $currComp) {
            ...
            Stop-Service -Name wuauserv    #### Need to get status first, then perform action based on current state? (If stopped, start, otherwise restart... ?)
...
if ($errorList.Count -gt 0) {
    ...
    New-Item -Path ".\" -ItemType "file" -Name $errorLogFileName *>$null
    $outputString = "`r`n** Unreachable Nodes **"
    Add-Content -Path $errorLogPath -Value $outputString
    $errorList | Select-Object @{ n = 'Hostname-Key_Value' ; e = {$_.Hostname_ValueName}},
                               @{ n = 'Exceptions Generated' ; e = {$_.Exception}} | Add-Content -Path ".\$errorLogFileName"
