...
if ($instructionsOn) {
    ...
    Write-Output "`t`t3. Right-click on 'PolFileEditor.dll', select properties, and if blocked, unblock.**`n"
    Write-Output "`t`t`t There will be an option to unblock at the bottom of the properties window if blocked."
    Write-Output "`t`t`t If there is no option to unblock, the assembly is not blocked.`n"
    Write-Output "`t`t4. In Administrator Powershell command prompt, enter the following commands:`n"
    ...
    Write-Output "`t*If using Windows 7 or older OS, use C:\Windows\System32\WindowsPowerShell\v1.0\Modules`n"
    Write-Output "`t**If the assembly file had to be unblocked and there is a load error after completing the remaining`n"
    Write-Output "`t   steps, reboot the machine and attempt to re-run the script.`n`n"
    ...
    Write-Output "`t`te.g. `$WSUS_Server = `"10.100.75.13`" & `$WSUS_Server_Port = `"8531`"`n"
...
$clearWSUSPolicies = $null
do {
    $clearWSUSPolicies = Read-Host -Prompt "[C]lear or [S]et WSUS Policies? [Default=Set]"
    if ($clearWSUSPolicies -and $clearWSUSPolicies -ne "S" -and $clearWSUSPolicies -ne "C") {
        Write-Output "`nPlease enter 'S' to set WSUS Group Policy settings, or 'C' to clear them (i.e. set to 'Not Configured').`n"
    }
    elseif ($clearWSUSPolicies -eq "q") { exit }
}
while ($clearWSUSPolicies -and $clearWSUSPolicies -ne "S" -and $clearWSUSPolicies -ne "C")
if (!$clearWSUSPolicies -or $clearWSUSPolicies -eq "S") { $clearWSUSPolicies = $false }
else { $clearWSUSPolicies = $true }

if (!$clearWSUSPolicies) {
    $WSUS_Server = "10.100.75.13"
    if ($WSUS_Server -eq "WSUS_Server") { $WSUS_Server = Read-Host -Prompt "WSUS Server IP Address" }
    if ($WSUS_Server -eq "q") { exit }

    $WSUS_Server_Port = "8530"
    if ($WSUS_Server_Port -eq "WSUS_Server_Port") { $WSUS_Server_Port = Read-Host -Prompt "WSUS Server Port [Default=8531]" }
...
$comps | ForEach-Object {
    ...
    # Update Keys in "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate" 
    ...
    if (!$clearWSUSPolicies) {    
        ...
        Catch { $errorList.Add( @{ 'Hostname_ValueName' = "$($currComp):ElevateNonAdmins" ; 'Exception' = $_.Exception.Message } ) }
    }
    else {    
        Get-PolicyFileEntry -Path $path -all | where-object { $_.key -eq $key } | Select-Object ValueName -OutVariable Names_WU_ToClear >$null
        $Names_WU_ToClear | ForEach-Object { 
            $currPolRegToClear = $_.ValueName
            Try { 
                Remove-PolicyFileEntry -Path $path -Key $key -ValueName $_.ValueName -ErrorAction Stop
            }
            Catch { $errorList.Add( @{ 'Hostname_ValueName' = "$($currComp):$($currPolRegToClear)" ; 'Exception' = $_.Exception.Message } ) }
        }
    }
    ...
    if (!$clearWSUSPolicies) {
        ...
        Get-PolicyFileEntry -Path $path -all | where-object { $_.key -eq $key } | Select-Object ValueName -OutVariable Names_WUAU_ToClear >$null
        $Names_WUAU_ToClear | ForEach-Object { 
            $currPolRegToClear = $_.ValueName
            Try { 
                Remove-PolicyFileEntry -Path $path -Key $key -ValueName $_.ValueName -ErrorAction Stop
            }
...
else { write-host "`nWSUS Settings Successfully $(if ($clearWSUSPolicies) { "Cleared" } else { "Updated" } ). Press enter to exit..." }
